...
**Token-level** B-trends and poem-level C/D trends use **temporal intervals**
(default: **balanced** bins — similar poem or stanza counts per bin via
``utils.adaptive_temporal_binning.balanced_temporal_binning``; optional legacy
**calendar merge** mode matches ``08_adaptive_binning.py``). Rows without a
binnable date are excluded from those series. **Inferential tests** do not use
raw token rows; they use **one row per poem** (continuous ``prop_*``) or **one row
per stanza** (binary indicators for modal labels).

**Poem-level** continuous proportions (``prop_*``, one row per poem) support
descriptive plots. **E** outputs are trimmed to ``E_top_authors_by_poem_share.csv``;
authors active in both ``pre_2022`` and ``post_2022`` with enough poems per era get
per-author pre/post plots under ``author_pre_post_2022/``. **2022 breakpoint inference** (Mann–Whitney, Cohen's d, Benjamini–Hochberg) is written under
``pre_post_2022_inference/``: corpus-wide and **per major language** (Ukrainian, Russian, Qirimli)
for both poem-level ``prop_*`` and stanza-level modal pronoun indicators.

Descriptive ``period`` matches the binary cut: **year < 2022** →
``pre_2022``, **year ≥ 2022** → ``post_2022`` (same as ``period_binary_2022``).

F_* tables that report binomial-style ``pct`` also include the **Wilson score
lower bound** (two-sided 95% interval endpoint) for conservative ranking across
unequal ``n``; row order follows that bound where a leaderboard is implied.

Trend plots (B–D, E author bridges) temporarily omit **third person** (``3rd Singular`` /
``3rd Plural``, and ``prop_3rd`` on E left panel); C/D matching interval CSVs follow the
plots. F_* tables and Mann–Whitney inference CSVs still include third person unless
otherwise noted.
"""
...
try:
    ...
    _fdr_bh = None  # type: ignore[misc, assignment]

try:
    from statsmodels.stats.multitest import multipletests as _multipletests_bh
except ImportError:
    _multipletests_bh = None  # type: ignore[misc, assignment]
...
DEFAULT_OUTPUT = ROOT / "outputs" / "13_descriptive_statistics"
DEFAULT_LAYER0 = ROOT / "data" / "To_run" / "00_filtering" / "layer0_poems_one_per_row.csv"

# Descriptive tables / F_* and poem-level inference share this axis.
PERIOD_DESCRIPTIVE_ORDER = ("pre_2022", "post_2022")
PROP_FEATURES = ["prop_1st", "prop_2nd", "prop_3rd", "prop_plural", "prop_pro_drop"]
# E author bridge left panel only; inference CSVs still use full ``PROP_FEATURES``.
...
# Per-language stanza tests: need enough stanzas in each era (pronoun-bearing stanzas).
MIN_STANZAS_PER_PERIOD_FOR_LANGUAGE_INFERENCE = 10
# 「超过5首」per period: strictly more than 5 poems → at least 6 per era.
MIN_POEMS_EACH_PERIOD_FOR_AUTHOR_PRE_POST = 6

# Wilson score interval (two-sided): lower bound as conservative % for ranking small-n slices.
...
# Canonical display labels for 1st/2nd/3rd × Singular/Plural (aligned with UD-style person+number).
SIXCODE_TO_DISPLAY = {
    "1sg": "1st Singular",
    "1pl": "1st Plural",
    "2sg": "2nd Singular",
...
# Post-hoc poem primary = mode of per-row labels below; order for tables/plots.
PN_KEY_ORDER = [
    "1st Singular",
    "1st Plural",
    "2nd Singular",
...
# Excluded from F perspective summaries (still in raw data & per-poem CSV).
DISPLAY_OMIT_PERSPECTIVE_PRIMARY = frozenset({"Impersonal/Other", "Other/ambiguous"})
# Third person: hidden from B stanza trends, C/D poem-perspective trends, E bridge plots, and matching trend CSVs.
...
def _wilson_lower_bound_pct(
    ...
    *,
    ...
    """Lower endpoint of the two-sided Wilson score interval for Binomial(trials, p), as % in [0, 100]."""
    t = float(trials)
...
def _period_pre_post_2022(y) -> str:
    ...
    if pd.isna(y):
        return "unknown"
    return MW_PERIOD_POST if int(y) >= 2022 else MW_PERIOD_PRE
...
def attach_adaptive_intervals(
    ...
    **balanced** (default): chronological bins with similar counts per bin
    (poems, or stanzas if ``balance_by=\"stanzas\"`` and ``stanza_index`` exists).
    **min_calendar**: legacy multi-month buckets merged until ``min_poems``.

    Reference date per poem: earliest in-frame ``date``, else mid-year from
    ``year``. Merged columns: ``interval_id``, ``interval_label``,
    ``interval_n_poems``, ``interval_start_date``, ``interval_end_date``.
    """
...
def pronoun_row_pn_label(row: pd.Series) -> str:
    ...
    Avoids degenerate keys like ``2nd`` when ``number`` is missing (those become
    ``Other/ambiguous``).
    """
...
def compute_poem_perspective(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Input: pronoun_annotation.csv (one row per pronoun per stanza)
    Output: one row per poem_id with perspective fields (no GPT prompt; fully auditable).
    """
...
def write_author_pre_post_bridge_plots(
    ...
    *,
    ...
    """One figure per author with pre/post 2022 poems in both eras (≥ ``min_poems_each_period`` each).

    Left: poem-level mean token proportions. Right: stanza-level % by modal person×number
    (equal weight per stanza with ≥1 pronoun token), same categories as B-trends.
    """
    if poem_props.empty or "author" not in poem_props.columns:
        return 0
...
# ---------------------------------------------------------------------------
# Pre/post 2022 — pronoun inference (poem- and stanza-level units)
# ---------------------------------------------------------------------------
...
def _mann_whitney_continuous_block(
    ...
    *,
    ...
    """Two-sided Mann–Whitney: post vs pre on each numeric column (one row per poem or binary stanza)."""
...
def write_pre_post_2022_pronoun_inference(
    ...
    **Poem unit**: continuous ``prop_*`` (one poem ≥ ``min_n_pronouns`` tokens); overall + per
    ``language_clean`` in ``MAJOR_LANGUAGES``.
    **Stanza unit**: modal person×number and modal number indicators; overall + per language.
    """
...
# ---------------------------------------------------------------------------
# B  – Pronoun person / number trend line charts
# ---------------------------------------------------------------------------
...
def _mode_with_tie_order(series: pd.Series, preference: list[str]) -> str:
    ...
    tied = counts[counts.eq(top_n)].index.tolist()
    if len(tied) == 1:
        return str(tied[0])
...
def build_stanza_unit_pronoun_frame_simple(df: pd.DataFrame) -> pd.DataFrame:
    """One row per (poem_id, stanza_index): modal person×number; uses ``period`` on input rows (no ``interval_id``)."""
    if "stanza_index" not in df.columns:
        return pd.DataFrame(
            columns=[
                "poem_id",
...
def _year_vline_x_ordinal(
    ...
    """First chronological bin whose ``start_date`` is on/after Jan 1 ``year`` → boundary at ``k - 0.5``."""
    ts = pd.Timestamp(int(year), 1, 1)
    iv = interval_df.set_index("interval_id")
...
def _plot_adaptive_interval_trend(
    ...
    *,
    ...
    """Line chart over time bins: default **ordinal** (equal width per bin); optional **density** x."""
...
def _attach_interval_metadata(pct: pd.DataFrame, interval_df: pd.DataFrame) -> pd.DataFrame:
    """Join ``interval_label`` / dates / ``n_poems`` onto a crosstab-derived frame indexed by ``interval_id``."""
    if pct.empty:
...
# ---------------------------------------------------------------------------
# D  – Poem perspective by language
# ---------------------------------------------------------------------------
...
def write_extra_tables(df: pd.DataFrame, poems: pd.DataFrame, out: Path):
    ...
    # F2: person by language and period
    rows = []
    for lang in MAJOR_LANGUAGES:
    ...
    # F3: pro-drop rate by language and period
    pro_rows = []
    for lang in MAJOR_LANGUAGES:
        for period in PERIOD_DESCRIPTIVE_ORDER:
    ...
    # F4: poem perspective (post-hoc primary) by language and period
    ...
    # F5: average stanzas per poem by period
    stanza_rows = []
    for period in PERIOD_DESCRIPTIVE_ORDER:
...
def main():
    ...
    print("Writing F – supplementary tables …")
