...
DEFAULT_OUT = ROOT / "outputs" / "16_three_period_models_v2"
...
def _sixway(person: str, number: str) -> str:
    pmap = {"1st": "1", "2nd": "2", "3rd": "3"}
    nmap = {"Singular": "sg", "Plural": "pl"}
    if person in pmap and number in nmap:
        return f"{pmap[person]}{nmap[number]}"
    return ""
...
def load_and_filter(path: Path, layer0_path: Path) -> pd.DataFrame:
    ...
    df["person_number"] = [_sixway(p, n) for p, n in zip(df["person"], df["number"])]
