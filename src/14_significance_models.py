...
This script complements `13_descriptive_statistics.py` by fitting regression models
with robust/clustered uncertainty, rather than only rank tests.

Outputs are written to `outputs/14_significance_models/`:
- poem_level_logit_ols.csv
- poem_level_logit_ols_by_language.csv
- stanza_plural_glm.csv
- stanza_plural_glm_by_language.csv
- stanza_pn_one_vs_rest_glm.csv
- stanza_pn_one_vs_rest_glm_by_language.csv
- poem_level_segmented_2014_2022.csv
"""
...
DEFAULT_OUTPUT = ROOT / "outputs" / "14_significance_models"
...
def _period_pre_post_2022(y) -> str:
    if pd.isna(y):
        return "unknown"
    return PERIOD_POST if int(y) >= 2022 else PERIOD_PRE
