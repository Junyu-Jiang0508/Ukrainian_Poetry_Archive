...
def build_pipeline_catalog() -> list[PipelineStage]:
    ...
    src = root / "src"
    rows: list[tuple[str, Path, str, str, tuple[str, ...], tuple[str, ...]]] = [
        ...
        (
            "02a",
            src / "02_modeling_significance_core_contrasts.py",
            "Attention Allocation Core Contrasts",
            "Closed 1st/2nd-person four-cell allocation: mixed-effects binomial primary (author random intercept) + conditional-logit sensitivity.",
            (),
            (),
        ),
        (
        ...
        (
            "02b3",
            src / "02_modeling_q1_per_cell_glm.py",
            "Absolute Salience GLM (finite-verb offset)",
            "Same script, --exposure-type=n_finite_verbs; absolute pronoun rates per finite-verb syntactic slot (sensitivity).",
            ("--exposure-type", "n_finite_verbs"),
            (),
        ),
        (
            "02b4",
            src / "02_modeling_q1_per_cell_glm.py",
            "Absolute Salience GLM (FV excl imperative)",
        ...
        ("02brat", src / "02_modeling_ratio_indices.py", "Ratio Builder", "Build poem-level pronoun ratio index table and exclusion audits.", (), ()),
        ("02bratpop", src / "02_modeling_ratio_q1_binomial.py", "Ratio Q1 Binomial", "Population-level ratio shifts via clustered binomial GLM + coprimary outputs.", (), ("02brat",)),
        ("02bratq1b", src / "02_modeling_ratio_q1b_within_author_fe.py", "Ratio Q1b Author FE", "Within-author FE binomial ratio models with sparsity audits + bootstrap deltas.", (), ("02brat",)),
        ("02bratq2", src / "02_modeling_ratio_q2_hierarchical.py", "Ratio Q2 Hierarchical", "Hierarchical binomial ratio models with author random slopes.", (), ("02brat",)),
    ...
    return [
        ...
        for stage_id, script_path, title, description, extra_args, depends_on in rows
    ]
